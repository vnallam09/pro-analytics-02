...
class DataScrubber:
    """A reusable utility class for performing common data cleaning and preparation tasks.

    This class wraps a pandas DataFrame and provides chainable methods for data cleaning operations
    including consistency checks, duplicate removal, missing value handling, outlier filtering,
    column management, string formatting, and date parsing.

    Attributes
    ----------
    df : pd.DataFrame
        The pandas DataFrame being cleaned and processed.
    ...
    def check_data_consistency_before_cleaning(self) -> dict[str, pd.Series | int]:
        ...
        Returns:
            dict: Dictionary with 'null_counts' (pd.Series) and 'duplicate_count' (int).
        """
    ...
    def check_data_consistency_after_cleaning(self) -> dict[str, pd.Series | int]:
        ...
        Returns:
            dict: Dictionary with 'null_counts' (pd.Series) and 'duplicate_count' (int), expected to be zero for each.
        """
        null_counts = self.df.isnull().sum()
        duplicate_count = self.df.duplicated().sum()
    ...
    def filter_column_outliers_iqr(self, column: str, multiplier: float = 1.5) -> pd.DataFrame:
        """Filter outliers in a specified column using the Interquartile Range (IQR) method.

        The IQR method defines outliers as values that fall below Q1 - (multiplier * IQR)
        or above Q3 + (multiplier * IQR), where Q1 and Q3 are the first and third quartiles.
        ...
        multiplier : float, optional
            Multiplier for IQR to determine outlier bounds. Default is 1.5.

        Returns
        -------
        pd.DataFrame
            Updated DataFrame with outliers filtered out.
